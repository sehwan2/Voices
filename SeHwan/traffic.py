...
def index():
    if request.method == 'POST':
        ...
        if destination_latitude is not None and destination_longitude is not None:
            ...
            # 지도를 HTML 파일로 저장 (templates 폴더의 절대 경로를 직접 지정)
            templates_folder = os.path.join(os.path.dirname(__file__), 'templates')
            html_file_path = os.path.join(templates_folder, 'tmap_route_map_with_traffic_light.html')
            tmap_map.save(html_file_path)
